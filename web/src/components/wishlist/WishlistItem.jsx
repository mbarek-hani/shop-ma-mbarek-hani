...
export default function WishlistItem({ item, onAddToCart, onRemove }) {
    ...
    <li className="py-4">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-sm text-slate-600 dark:text-slate-400">
            <div className="flex items-center gap-3 flex-wrap">
              <Button variant="success" onClick={() => onAddToCart(item)}>Ajouter au panier</Button>
              <Button variant="danger" className="flex items-center gap-2" onClick={() => onRemove(item.id)}>
                <Trash className="w-4 h-4" /> <span className="hidden sm:inline">Retirer</span>
              </Button>
            </div>
